...
public class DeviceSizeUtil {
  ...
  public static float dpToPixel(float dp) {
    return (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dp,
        EditorApp.Companion.getInstance().getApp().getResources().getDisplayMetrics());
  }
  ...
  public static float getPixelDensity() {
    ...
    DisplayMetrics metrics = EditorApp.Companion.getInstance().getApp().getResources().getDisplayMetrics();
    sPixelDensity = metrics.density;
    return sPixelDensity;
  }

  private static void initScreenSize() {
    Context appContext = EditorApp.Companion.getInstance().getApp();
