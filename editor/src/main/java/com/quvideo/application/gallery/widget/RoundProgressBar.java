...
public class RoundProgressBar extends View {
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    ovalRect = new RectF(centre - radius, centre - radius, centre + radius,
        centre + radius); // 用于定义的圆弧的形状和大小的界限

    switch (style) {
      case STROKE:
        paint.setStyle(Paint.Style.STROKE);
        canvas.drawArc(ovalRect, 0, 360 * progress / max, false, paint); // 根据进度画圆弧
        break;
      case FILL:
        paint.setStyle(Paint.Style.FILL_AND_STROKE);
        if (progress != 0) {
          canvas.drawArc(ovalRect, 0, 360 * progress / max, true, paint); // 根据进度画圆弧
        }
        break;
      case FILL_UP:
        paint.setStyle(Paint.Style.FILL_AND_STROKE);
        if (progress != 0) {
          ...
          canvas.drawArc(ovalRect, angle, 360 * progress / max, false, paint);
        }
        break;
    }
