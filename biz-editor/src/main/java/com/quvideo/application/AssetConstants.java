...
public class AssetConstants {
  ...
  static {
    ...
    // theme
    mThemeTemplateList.add("assets_android://quvideo/theme/0x0100000000000164.zip");
    // trans
    ...
    mSubtitleTemplateList.add("assets_android://quvideo/subtitle/0x0900000000000276.xyt");
    mSubtitleTemplateList.add("assets_android://quvideo/subtitle/0x090000000000030C.xyt");
    mSubtitleTemplateList.add("assets_android://quvideo/subtitle/0x0900000000080084.xyt");
    // multisubtitle
   ...
   * 这种判断是非正规的，实际的id不一定会这样，仅测试素材是这样的规则。开发者请勿这样使用
   */
  public static EditFilterTemplate[] getFrameXytListByType(XytType xytType) {
    List<EditFilterTemplate> result = new ArrayList<>();
    //ArrayList<EditFilterTemplate> result = new ArrayList<>(Arrays.asList(AssetConstants.TEST_SUBTITLE_TID));
    if (xytType == XytType.Transition
        || xytType == XytType.Filter
    ...
    for (Long ttid : keySet) {
      xytInfo = temp.get(ttid);
      if (xytInfo != null && isCurrentFrameType(xytType, xytInfo)) {
        if (xytType != XytType.SubFx || QEXytUtil.isSupportSubFx(xytInfo.filePath)) {
          result.add(new EditFilterTemplate(ttid));
        }
      }
   ...
   * 这种判断是非正规的，实际的id不一定会这样，仅测试素材是这样的规则。开发者请勿这样使用
   */
  private static boolean isCurrentFrameType(XytType xytType, XytInfo xytInfo) {
    switch (xytType) {
      case Theme:
      ...
      case Subtitle:
        return xytInfo.ttidHexStr.contains("0x09") && !xytInfo.ttidHexStr.contains("0x09000000001");
      case SubtitleAnimIn:
