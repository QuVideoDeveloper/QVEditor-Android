...
class EditorApp private constructor() {
  ...
  fun init(
    ...
    var builder: Builder = Builder(licensePath)
    QEEngineClient.init(app, builder.build())
    SuperEditManager.initAIComponent(app)
    SuperEditManager.initAICache(app, builder)
    if (SuperEditManager.isHadSuperEdit()) {
      SuperEditManager.checkCache();
    }
    val lastCersionCode = DemoSharedPref.getInstance().lastVersionCode
