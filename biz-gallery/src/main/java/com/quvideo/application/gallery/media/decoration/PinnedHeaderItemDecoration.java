package com.quvideo.application.gallery.media.decoration;
...
import com.quvideo.application.gallery.media.decoration.callback.OnHeaderClickListener;
import com.quvideo.application.gallery.media.decoration.callback.OnItemTouchListener;
import com.quvideo.application.gallery.media.decoration.utils.DividerHelper;
...
public class PinnedHeaderItemDecoration extends RecyclerView.ItemDecoration {
  ...
  @Override public void getItemOffsets(@NonNull final Rect outRect, @NonNull final View view,
    ...
    if (mDrawable == null) {
      mDrawable = ContextCompat.getDrawable(parent.getContext(),
          mDividerId != 0 ? mDividerId : R.drawable.divider);
    }
  ...
  @Override public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
    ...
    if (!mDisableDrawHeader
        && mPinnedHeaderView != null
        && mFirstVisiblePosition >= mPinnedHeaderPosition) {
      ...
      if (belowViewAdapterPosition > mFirstVisiblePosition && isPinnedHeader(parent,
          belowView)) {
        // 如果是标签的话，缓存的标签就要同步跟此标签移动
        // 根据belowView相对顶部距离计算出缓存标签的位移
        mPinnedHeaderOffset = belowView.getTop() - (mRecyclerViewPaddingTop
            + mPinnedHeaderView.getHeight()
            + mHeaderTopMargin);
   ...
   * 创建标签强制固定在顶部
   */
  @SuppressWarnings("unchecked") private void createPinnedHeader(final RecyclerView parent) {
    ...
    if (pinnedHeaderPosition >= 0 && mPinnedHeaderPosition != pinnedHeaderPosition) {
      ...
      final int maxHeight =
          parent.getHeight() - mRecyclerViewPaddingTop - recyclerViewPaddingBottom;
      // 不能超过maxHeight
      heightSize = Math.min(heightSize, maxHeight);

      // 因为标签默认宽度占满parent，所以宽度强制为RecyclerView的宽度减去padding
      final int widthSpec = View.MeasureSpec.makeMeasureSpec(parent.getWidth()
          - mRecyclerViewPaddingLeft
          - recyclerViewPaddingRight
          - mHeaderLeftMargin
          - mHeaderRightMargin, View.MeasureSpec.EXACTLY);
      ...
      if (mItemTouchListener == null && mHeaderClickListener != null) {
        ...
        try {
          ...
          final ArrayList<RecyclerView.OnItemTouchListener> touchListeners =
              (ArrayList<RecyclerView.OnItemTouchListener>) field.get(parent);
