...
public class OnItemTouchListener implements RecyclerView.OnItemTouchListener {
  ...
  public void setClickBounds(int id, View view) {
    ...
    if (mBoundsArray.get(id) == null) {
      bounds = new ClickBounds(view, view.getLeft(), view.getTop(),
          view.getLeft() + view.getMeasuredWidth(), view.getTop() + view.getMeasuredHeight());
      mBoundsArray.put(id, bounds);
    } else {
      bounds = mBoundsArray.get(id);
      bounds.setBounds(view.getLeft(), view.getTop(), view.getLeft() + view.getMeasuredWidth(),
  ...
  @Override public boolean onInterceptTouchEvent(@NonNull final RecyclerView rv,
  ...
  private void shouldIntercept(MotionEvent e) {
    ...
    for (int i = 0; i < mBoundsArray.size(); i++) {
      ...
      boolean inside = downX >= bounds.getLeft()
          && downX <= bounds.getRight()
          && downY >= bounds.getTop()
      ...
      if (inside) {
        ...
        if (mTmpBounds == null) {
          mTmpBounds = bounds;
        } else if (bounds.getLeft() >= mTmpBounds.getLeft()
            && bounds.getRight() <= mTmpBounds.getRight()
            && bounds.getTop() >= mTmpBounds.getTop()
            && bounds.getBottom() <= mTmpBounds.getBottom()) {
  ...
  private class GestureListener extends GestureDetector.SimpleOnGestureListener {
    ...
    @Override public void onLongPress(MotionEvent e) {
      ...
      if (!mDisableHeaderClick
          && mIntercept
          && mHeaderClickListener != null
          && mAdapter != null
