package com.quvideo.application.gallery.widget.crop;
...
import com.quvideo.application.gallery.widget.crop.handle.Handle;
import com.quvideo.application.gallery.widget.crop.util.AspectRatioUtil;
import com.quvideo.application.gallery.widget.crop.util.HandleUtil;
...
public class CropImageView extends AppCompatImageView {
  ...
  @SuppressWarnings("unused") private static final String TAG = CropImageView.class.getName();

  @SuppressWarnings("unused") public static final int GUIDELINES_OFF = 0;
  ...
  @NonNull private RectF mBitmapRect = new RectF();
  ...
  @NonNull private PointF mTouchOffset = new PointF();
  ...
  private CropActionCallback cropActionCallback;
  ...
  public void setCropActionCallback(CropActionCallback callback) {
    this.cropActionCallback = callback;
  }
  ...
  @Override protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    mBitmapRect = new RectF(0, 0, right - left, bottom - top);
  ...
  /**
   * reset crop values and init status
   *
   * @param minPxInHor Minimum distance in pixels on  the x-coordinate
   * @param minPxInVer Minimum distance in pixels on  the y-coordinate
   */
  public void resetCrop(int minPxInHor, int minPxInVer) {
    mInited = false;
    setMinCropLengthPx(minPxInHor, minPxInVer);
  }

  public void setMinCropLengthPx(int minPxInHor, int minPxInVer) {
    //MIN_CROP_LENGTH_PX in x-coordinate & y-coordinate of Edge
    Edge.LEFT.setMinCropLengthHor(minPxInHor);
    Edge.TOP.setMinCropLengthHor(minPxInHor);
    Edge.RIGHT.setMinCropLengthHor(minPxInHor);
    Edge.BOTTOM.setMinCropLengthHor(minPxInHor);
    Edge.LEFT.setMinCropLengthVer(minPxInVer);
    Edge.TOP.setMinCropLengthVer(minPxInVer);
    Edge.RIGHT.setMinCropLengthVer(minPxInVer);
    Edge.BOTTOM.setMinCropLengthVer(minPxInVer);
  }
  ...
  public Bitmap getCroppedImage() {
    ...
    final float cropWidth =
        Math.min(Edge.getWidth() / scaleX, originalBitmap.getWidth() - cropX);
    final float cropHeight =
        Math.min(Edge.getHeight() / scaleY, originalBitmap.getHeight() - cropY);
  ...
  private void initCropWindow(@NonNull RectF bitmapRect) {
    ...
    if (mFixAspectRatio) {
      ...
      // Initialize crop window to have 0 padding w/ respect to Drawable's bounds.
      final float horizontalPadding = 0.f * bitmapRect.width();
      final float verticalPadding = 0.f * bitmapRect.height();
  ...
  private void initCropWindowWithFixedAspectRatio(@NonNull RectF bitmapRect) {
    ...
    if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > getTargetAspectRatio()) {
      ...
      final float cropWidth =
          AspectRatioUtil.calculateWidth(bitmapRect.height(), getTargetAspectRatio());
      ...
      final float cropHeight =
          AspectRatioUtil.calculateHeight(bitmapRect.width(), getTargetAspectRatio());
  ...
  private void drawCorners(@NonNull Canvas canvas) {
    ...
    // Absolute value of the offset by which to draw the corner line such that
    // its inner edge is flush with the border's inner edge.
    final float lateralOffset = (mBorderThickness - mCornerThickness) / 2f;
    // Absolute value of the offset by which to start the corner line such that
    // the line is drawn all the way to form a corner edge with the adjacent side.
    final float startOffset = mBorderThickness / 2f;
  ...
  private boolean shouldGuidelinesBeShown() {
    return ((mGuidelinesMode == GUIDELINES_ON) || ((mGuidelinesMode == GUIDELINES_ON_TOUCH) && (
        mPressedHandle
            != null)));
  }
  ...
  private void onActionUp() {
    if (cropActionCallback != null) {
      cropActionCallback.onCrop();
    }
    if (mPressedHandle != null) {
      mPressedHandle = null;
      invalidate();
  ...
  /**
   * @return 返回一个rect，代表ltrb每个坐标的万分比
   */
  public RectF getCroppedRect() {
    RectF rect = new RectF();
    rect.left = (int) (Edge.LEFT.getCoordinate() * 10000 / mBitmapRect.width());
    rect.top = (int) (Edge.TOP.getCoordinate() * 10000 / mBitmapRect.height());
    rect.right = (int) (Edge.RIGHT.getCoordinate() * 10000 / mBitmapRect.width());
    rect.bottom = (int) (Edge.BOTTOM.getCoordinate() * 10000 / mBitmapRect.height());

    return rect;
  }

  public interface CropActionCallback {
    void onCrop();
  }
}
